...
const users = [
  ['cliente@hermes.demo', '/cliente/inicio', ['Inicio', 'Explorar', 'Reservas', 'Perfil']],
  ['agente@hermes.demo', '/agente/inicio', ['Inicio', 'Operaciones', 'Escanear', 'Incidentes', 'Perfil']],
  ['admin@hermes.demo', '/admin/dashboard', ['Resumen', 'Flota', 'Reservas', 'Operaciones', 'Más']],
  ['superadmin@hermes.demo', '/super-admin/dashboard', ['Resumen', 'Empresas', 'Planes', 'Suscripciones', 'Plataforma']],
];
const routes = {
  ...
  admin: ['dashboard', 'flota', 'flota/vehicle-001', 'reservas', 'operaciones', 'clientes', 'inspecciones', 'contratos', 'facturacion', 'mantenimiento', 'configuracion'],
  'super-admin': ['dashboard', 'empresas', 'empresas/tenant-001', 'planes', 'suscripciones', 'plataforma'],
};
...
try {
  ...
  for (const [email, home] of users) {
    ...
    await until(`location.pathname === ${JSON.stringify(home)} && document.body.innerText.includes('Cerrar sesión')`);
  ...
  for (const [role, paths] of Object.entries(routes)) {
    await evaluate(`localStorage.setItem('hermes.mock.session', ${JSON.stringify(role === 'super-admin' ? 'superadmin@hermes.demo' : role + '@hermes.demo')})`);
